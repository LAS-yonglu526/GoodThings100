...
export async function createList(id: string, title: string, themeType: string, iconEmoji: string, coverColor: string, itemLimit: number) {
  await getDB().runAsync('INSERT INTO lists (id, userId, title, themeType, iconEmoji, coverColor, itemLimit, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [id, '', title, themeType, iconEmoji, coverColor, itemLimit, new Date().toISOString()]);
}
...
export async function importData(lists: GoodList[], items: GoodItem[]) {
  ...
  for (const l of lists) await d.runAsync('INSERT INTO lists (id, userId, title, themeType, iconEmoji, coverColor, itemLimit, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [l.id, l.userId || '', l.title, l.themeType, l.iconEmoji, l.coverColor, l.itemLimit, l.createdAt]);
